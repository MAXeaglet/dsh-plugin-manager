...
export function listPlugins(profileName) {
  ...
  for (const name of bundles) {
    ...
    const insertIds = bundleInsertIds(dir, name);
    // disabled = every real inserted row is marked disabled in the user patch
    let disabled = false;
    if (insertIds.length) {
      disabled = insertIds.every((rid) => byId.get(rid)?.disabled === true);
    } else {
      disabled = byId.get(id)?.disabled ?? false;
    }
    // track config from any real row
    let config;
    for (const rid of insertIds) { if (byId.get(rid)?.config !== undefined) { config = byId.get(rid).config; break; } }
