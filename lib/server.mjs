...
import { listProfiles, listPlugins, setPluginDisabled, setBundle, setBundleOrder, exportProfile, installPlugin, profileDir, profilesDir, dshStatus, startDsh, stopDsh, setPluginConfig, searchNpm, updatePlugin, checkUpdates, checkDshUpdate, updateDsh, normalizeRepoUrl } from "./profiles.mjs";
    ...
    if (path === "/api/open-repo" && req.method === "POST") {
      ...
      if (url) {
        ...
        const openError = await new Promise((resolve) => {
          try { opener.exec(cmd, (err) => resolve(err ? String(err.message || err) : null)); }
          catch (e) { resolve(String(e.message || e)); }
        });
        if (openError) return json(res, 200, { ok: false, error: openError });
        return json(res, 200, { ok: true, url });
      }
      return json(res, 200, { ok: false, error: "no repository" });
    }
    if (path === "/api/open-dshbase" && req.method === "POST") {
      const opener = await import("node:child_process");
      try {
        if (process.platform === "win32") opener.exec("start https://dshbase.com");
        else if (process.platform === "darwin") opener.exec("open https://dshbase.com");
        else opener.exec("xdg-open https://dshbase.com");
        return json(res, 200, { ok: true, url: "https://dshbase.com" });
      } catch (e) {
        return json(res, 200, { ok: false, error: String(e.message || e) });
      }
    }
    if (path === "/api/check-dsh") {
      return json(res, 200, checkDshUpdate());
    }
    if (path === "/api/update-dsh" && req.method === "POST") {
      const result = updateDsh();
      return json(res, result.ok ? 200 : 500, result);
    }
    if (path === "/api/update-plugin" && req.method === "POST") {
      const body = await readBody(req);
      const { profile, name } = body;
      if (!profile || !name) return json(res, 400, { error: "profile+name required" });
      const result = updatePlugin(profile, name);
      return json(res, result.ok ? 200 : 500, result);
    }
    if (path === "/api/updates") {
      const profile = url.searchParams.get("profile");
      if (!profile) return json(res, 400, { error: "profile required" });
      return json(res, 200, checkUpdates(profile));
    }
    if (path === "/api/readme") {
      const profile = url.searchParams.get("profile");
