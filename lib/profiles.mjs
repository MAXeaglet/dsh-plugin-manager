...
export function listPlugins(profileName) {
  ...
  for (const entry of patchEntries) {
    ...
    if (typeof entry.id === "string") {
      // A patch row with neither disabled nor config is an inert materialized
      // copy of a bundle insert (dsh rewrites them into this file on boot).
      // Show only rows the user actually acted on.
      const hasDisabled = entry.disabled === true || entry.disabled === false;
      const hasConfig = entry.config !== undefined;
      if (allInsertIds.has(entry.id)) {
        rowDisabled.set(entry.id, entry.disabled === true);
        if (hasConfig) rowConfig.set(entry.id, entry.config);
        continue;
      }
      if (!hasDisabled && !hasConfig) continue; // materialized copy — skip
      byId.set(entry.id, {
        id: entry.id, name: entry.name ?? entry.id, source: "patch",
        disabled: entry.disabled === true, kind: "row",
        ...(hasConfig ? { config: entry.config } : {})
      });
    }
  }
...
export function searchNpm(query) {
  try {
    const res = spawnSync("npm", ["search", query, "--json"], { encoding: "utf8", timeout: 30000, shell: process.platform === "win32" });
    ...
    return { error: "npm search failed" };
  } catch (e) {
    return { error: String(e.message || e) };
  }
}

export function updatePlugin(profileName, packageName) {
  const dshBin = process.env.DSH_BIN || "dsh";
  const res = spawnSync(dshBin, ["plugin", "--profile", profileName, "update", packageName], {
    encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], shell: process.platform === "win32"
  });
  return { ok: res.status === 0, status: res.status, output: (res.stdout ?? "") + (res.stderr ?? "") };
}
...
export function normalizeRepoUrl(raw) {
  ...
  if (s.startsWith("github:")) s = "https://github.com/" + s.slice("github:".length);
  else if (s.startsWith("gitlab:")) s = "https://gitlab.com/" + s.slice("gitlab:".length);
  else if (s.startsWith("bitbucket:")) s = "https://bitbucket.org/" + s.slice("bitbucket:".length);
  // SCP-like / ssh forms
  else if (s.startsWith("git+ssh://git@github.com/")) s = "https://github.com/" + s.slice("git+ssh://git@github.com/".length);
  else if (s.startsWith("ssh://git@github.com/")) s = "https://github.com/" + s.slice("ssh://git@github.com/".length);
  else if (s.startsWith("git@github.com:")) s = "https://github.com/" + s.slice("git@github.com:".length);
  // git transport prefixes
  else if (s.startsWith("git+https://")) s = "https://" + s.slice("git+https://".length);
  else if (s.startsWith("git+http://")) s = "http://" + s.slice("git+http://".length);
  else if (s.startsWith("git://")) s = "https://" + s.slice("git://".length);
  else if (s.startsWith("git+ssh://")) { let rest = s.slice("git+ssh://".length); if (rest.startsWith("git@")) rest = rest.slice(4); s = "https://" + rest; }
  else if (s.startsWith("ssh://")) { let rest = s.slice("ssh://".length); if (rest.startsWith("git@")) rest = rest.slice(4); s = "https://" + rest; }
  if (s.startsWith("http://github.com/")) s = "https://" + s.slice("http://".length);
  while (s.endsWith("/")) s = s.slice(0, -1);
  if (s.endsWith(".git")) s = s.slice(0, -4);
  return s;
}

// lightweight semver compare: returns >0 if a newer than b, <0 if older, 0 equal.
// Handles major.minor.patch[-prerelease] without dependencies.
...
export function checkDshUpdate() {
  let local = null;
  try {
    const v = spawnSync("dsh", ["-V"], { encoding: "utf8", shell: process.platform === "win32", timeout: 15000 });
    if (v.status === 0 && v.stdout.trim()) local = v.stdout.trim();
  } catch {}
  let latest = null;
  try {
    const n = spawnSync("npm", ["view", "@deepseek-ai/dsh", "version"], { encoding: "utf8", shell: process.platform === "win32", timeout: 30000 });
    if (n.status === 0 && n.stdout.trim()) latest = n.stdout.trim();
  } catch {}
  return { local, latest, hasUpdate: !!local && !!latest && compareVersions(latest, local) > 0 };
}

export function updateDsh() {
  const res = spawnSync("npm", ["i", "-g", "@deepseek-ai/dsh"], {
    encoding: "utf8", shell: process.platform === "win32", timeout: 120000, stdio: ["ignore", "pipe", "pipe"]
  });
  return { ok: res.status === 0, status: res.status, output: (res.stdout ?? "") + (res.stderr ?? "") };
}
...
export function checkUpdates(profileName) {
  ...
  for (const p of listPlugins(profileName)) {
    ...
    let latest;
    try {
      const res = spawnSync("npm", ["view", p.name, "version"], { encoding: "utf8", shell: process.platform === "win32" });
      latest = res.status === 0 ? res.stdout.trim() : undefined;
    } catch { latest = undefined; }
    out.push({ id: p.id, name: p.name, local: p.version, latest: latest ?? null, hasUpdate: !!latest && compareVersions(latest, p.version) > 0 });
  }
  return out;
}
