...
import {
  ...
  installPlugin, exportProfile, dshStatus, startDsh, stopDsh, checkUpdates, updatePlugin, checkDshUpdate, updateDsh
} from "./profiles.mjs";
...
命令:
  ...
  update <name> [profile]          更新插件 (dsh plugin update)
  update-dsh                       检查并更新 DSH 本体
...
`);
  ...
  case "update": out(updatePlugin(pickProfile(1), rest[0])); break;
  case "update-dsh": {
    const s = checkDshUpdate();
    if (jsonFlag) { out(s); break; }
    if (!s.hasUpdate) { out({ text: "DSH 已是最新: " + (s.local || "?") }); break; }
    out({ text: "DSH " + s.local + " → " + s.latest + "，正在更新…" });
    const r = updateDsh();
    out({ text: r.ok ? "DSH 已更新到 " + (checkDshUpdate().local || "?") : "更新失败: " + (r.output || "").slice(0, 200) });
    break;
  }
